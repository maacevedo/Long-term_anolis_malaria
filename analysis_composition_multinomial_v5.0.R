...
#The difference between v1.1 and 1.2 is that in version 1.2 summer and winter are considered together.
...
#Input disease data
#anolis-plasmodium
ds=read.csv("Data/Matrix summer gund.csv",header=TRUE)

dw=read.csv("Data/Matrix winter gund.csv",header=TRUE)

###################################
#Data manipulation
###################################

ds.inf=subset(ds, ds$year>1997)#species id happened after 1997
dw.inf=subset(dw, dw$year>1997)#species id happened after 1997

#Format data for ordinal model
ds.inf$plasmodium=ifelse(ds.inf$fl==1,"fl",ifelse(ds.inf$azw==1,"azw",ifelse(ds.inf$azr==1,"azr",NA))) 
ds.inf$plasmodium[ds.inf$infeccion==0]="NI"

dw.inf$plasmodium=ifelse(dw.inf$fl==1,"fl",ifelse(dw.inf$azw==1,"azw",ifelse(dw.inf$azr==1,"azr",NA))) 
dw.inf$plasmodium[dw.inf$infeccion==0]="NI"

#reduce data frame to essential columns
ds.inf=ds.inf[,-c(1:5,7:8,11:18)]
dw.inf=dw.inf[,-c(1:5,7:8,11:17)]

ds.inf$year[ds.inf$year==1998]<-1998.5 #summer
ds.inf$year[ds.inf$year==2016]<-2016.5 #summer



#concatenate summer and winter into single data frame and sort for year
d.inf=rbind(ds.inf,dw.inf)
d.inf=d.inf[order(d.inf$year),]
d.inf$year=as.factor(d.inf$year)
